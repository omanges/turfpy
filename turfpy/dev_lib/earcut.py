...
def __linked_list(data, start, end, dim, clockwise):
    i = None
    last = None
...
def __isear_hashed(ear, minx, miny, size):
    a = ear.prev
    b = ear
    c = ear.next
...
def __cure_local_intersections(start, triangles, dim):
    do = True
    p = start
...
def __split_earcut(start, triangles, dim, minx, miny, size):
...
def __eliminate_holes(data, hole_indices, outer_node, dim):
    queue = []
    i = None
...
def __eliminatehole(hole, outer_node):
...
def __find_hole_bridge(hole, outer_node):
    do = True
    p = outer_node
    hx = hole.x
    hy = hole.y
...
# interlink polygon nodes in z-order
def __index_curve(start, minx, miny, size):
    do = True
    p = start
...
def __zorder(x, y, minx, miny, size):
...
def __point_in_triangle(ax, ay, bx, by, cx, cy, px, py):
    return (
...
def __deviation(data, hole_indices, dim, triangles):
    _len = len(hole_indices)
    has_holes = hole_indices and len(hole_indices)
    outer_len = hole_indices[0] * dim if has_holes else len(data)
...
def __signed_area(data, start, end, dim):
    sum = 0
    j = end - dim
